...
files.forEach(file => {
  if (path.extname(file) === '.astro') {
    ...
    // 3. Icons: design alargado (pill shape). We edit the CSS for .amenity-item
    ...
    content = content.replace(/\.reserve-button\s*\{[^}]*?\}/g, function(match) {
        if(match.includes('background')) {
           return match.replace(/background:\s*[^;]+;/g, 'background: #0f172a;')
                       .replace(/box-shadow:\s*[^;]+;/g, 'box-shadow: 0 4px 15px rgba(15, 23, 42, 0.4);');
        }
        return match;
    });
    ...
    content = content.replace(/\.reserve-button:hover\s*\{[^}]*?\}/g, function(match) {
        if(match.includes('background')) {
           return match.replace(/background:\s*[^;]+;/g, 'background: #1e293b;')
                       .replace(/box-shadow:\s*[^;]+;/g, 'box-shadow: 0 8px 25px rgba(15, 23, 42, 0.6);');
        }
        return match;
    });
    ...
    content = content.replace(/\.mobile-reserve-btn\s*\{[^}]*?\}/g, function(match) {
        if(match.includes('background')) {
           return match.replace(/background:\s*[^;]+;/g, 'background: #0f172a;')
                       .replace(/box-shadow:\s*[^;]+;/g, 'box-shadow: 0 -4px 20px rgba(15, 23, 42, 0.4);');
        }
        return match;
    });
